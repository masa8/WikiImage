...
#
# Gets image infos from wikipedia.
#
class WikiImage
  ...
  def get_images
    result=Array.new
    doc = Nokogiri::XML(open("http://jp.wikipedia.org/w/api.php?action=query&format=xml&prop=images&titles=#{@keyword}"))
 ...
 def get_imageinfo(files,props = :imageinfo)
    ...
    doc=Nokogiri::XML(open("http://jp.wikipedia.org/w/api.php?format=xml&action=query&prop=#{props.to_s}&titles=#{files}&iiprop=#{iiprop}"))
